...
def sim(z_i, z_j):
    ...
    norm_dot_product = (z_i * z_j).sum() / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        den_sum_k, den_sum_k_N = 0, 0
        for i in range(2*N):  
            den_sum_k += torch.exp(sim(z_k, out[i]) / tau)
            den_sum_k_N += torch.exp(sim(z_k_N, out[i]) / tau)

        total_loss -= 2 * torch.log(torch.exp(sim(z_k, z_k_N) / tau))
        total_loss += torch.log(den_sum_k - torch.exp(sim(z_k, z_k) / tau))
        total_loss += torch.log(den_sum_k_N - torch.exp(sim(z_k_N, z_k_N) / tau))
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left * out_right).sum(axis=1)
    pos_pairs /= (torch.linalg.norm(out_left, axis=1) * torch.linalg.norm(out_right, axis=1))
    pos_pairs = pos_pairs.reshape(-1, 1)
...
def compute_sim_matrix(out):
    ...
    norm = torch.linalg.norm(out, axis=1).reshape(-1, 1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = None
    sim_matrix = compute_sim_matrix(out).to(device)
    ...
    denom = None
    denom = exponential.sum(axis=1).reshape(-1, 1)
    ...
    numerator = torch.exp(torch.cat([pos_pairs, pos_pairs]) / tau).reshape(-1, 1).to(device) #(2N, 1)
