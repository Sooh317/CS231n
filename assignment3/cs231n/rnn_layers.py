...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_hdtanh = 1 - next_h**2 #(N, H)
    dLdtanh = dnext_h * dnext_hdtanh #(N, H)
    db = np.sum(dLdtanh, axis=0)    
    dx = np.matmul(dLdtanh, Wx.T)
    dWx = np.matmul(x.T, dLdtanh)
    dprev_h = np.matmul(dLdtanh, Wh.T)
    dWh = np.matmul(prev_h.T, dLdtanh)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    X = x.transpose(1, 0, 2)
    cache = {'Wx':Wx, 'Wh': Wh, 'x':x}
    T, N, D = X.shape
    H = b.shape[0]
    h = np.zeros((T, N, H))
    next_h = h0
    for i in range(T):
        next_h, (Wx, Wh, x, prev_hi, next_hi) = rnn_step_forward(X[i], next_h, Wx, Wh, b)
        h[i] = next_h
        cache['prev_h'+str(i+1)] = prev_hi
        cache['next_h'+str(i+1)] = next_hi
    h = h.transpose(1, 0, 2)
...
def rnn_backward(dh, cache):
    ...
    dprev_hi = 0
    for i in range(T):
        prev_hi = cache['prev_h' + str(T-i)]
        next_hi = cache['next_h' + str(T-i)]
        cachei = (Wx, Wh, X[-1-i], prev_hi, next_hi)
        dxi, dprev_hi, dWxi, dWhi, dbi = rnn_step_backward(dh[-1-i] + dprev_hi, cachei)
        dx[:, -1-i, :] = dxi
        db += dbi
        dWx += dWxi
        dWh += dWhi
...
def word_embedding_backward(dout, cache):
    ...
    (x, W) = cache
    dW = np.zeros_like(W)
    np.add.at(dW, x, dout) # it is similar to dW[x] += dout
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (Wx, Wh, x, prev_h, prev_c, i, f, o, g, next_h, next_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # Note that the derivative of the sigmoid function is sigma * (1 - sigma)
    # Also, the derivative of the tanh function is 1 - tanh**2

    (Wx, Wh, x, prev_h, prev_c, i, f, o, g, next_h, next_c) = cache
    N, D = x.shape
    N, H = prev_h.shape

    do = np.tanh(next_c) * dnext_h # (N, H)
    dnext_hdnext_c = o * (1 - np.tanh(next_c)**2) #(N, H)
    dnext_c += dnext_h * dnext_hdnext_c #(N, H)
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        cache_i = cache[i]
        dx_i, dprev_h, dnext_c, dWx_i, dWh_i, db_i = lstm_step_backward(dh[:, i, :] + dprev_h, dnext_c, cache_i)
        dx[:, i, :] = dx_i
        db += db_i
        dWh += dWh_i
        dWx += dWx_i
