...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, (j, k) in enumerate(zip([input_dim, *hidden_dims], [*hidden_dims, num_classes])):
            self.params['W'+str(i+1)] = weight_scale * np.random.randn(j, k)
            self.params['b'+str(i+1)] = np.zeros(k)

            if self.normalization and i < len(hidden_dims): 
                self.params['gamma'+str(i+1)] = np.ones(k)
                self.params['beta'+str(i+1)] = np.zeros(k)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_dropout:
                scores, dcache = dropout_forward(scores, self.dropout_param)

            cache_hist.append((cache, dcache))

        W = self.params['W' + str(self.num_layers)]
        b = self.params['b' + str(self.num_layers)]

        scores, last_cache = affine_forward(scores, W, b)
        ...
        dx, dw, db = affine_backward(dL, last_cache)
        w = self.params['W' + str(self.num_layers)]
