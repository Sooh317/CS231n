...
def svm_loss(x, y):
    ...
    num_train = x.shape[0]
    vals = np.reshape(x[np.arange(num_train), y], (-1, 1))
    diff = x - vals + 1
    diff[np.arange(num_train), y] = 0
    margin = np.maximum(0, diff)
    loss = np.sum(margin) / num_train
    dx = (diff > 0).astype(float)
    dx[np.arange(num_train), y] -= np.sum(diff > 0, axis=1)
    dx /= num_train
...
def softmax_loss(x, y):
    ...
    loss = (-np.sum(X[np.arange(num_train), y]) + np.sum(np.log(sums[:, 0]))) / num_train
    dx = exps / sums
